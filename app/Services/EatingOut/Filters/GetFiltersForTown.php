...
class GetFiltersForTown extends GetFilters
{
    protected ?EateryTown $town = null;
    ...
    public function setTown(EateryTown $town): self
    {
        $this->town = $town;
    ...
    {
        if ( ! $this->town) {
            throw new RuntimeException('Town not set');
        }
    ...
    protected function eateryQuery(): string
    {
        return Eatery::query()
            ->selectRaw('count(*)')
            ->where('town_id', $this->town->id) /** @phpstan-ignore-line */
            ->where('live', true)
            ->when(
                $this->relation,
